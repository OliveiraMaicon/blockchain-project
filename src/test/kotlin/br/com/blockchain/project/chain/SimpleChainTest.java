...
public class SimpleChainTest {
	...
	public void testBlockchain() {
		...
		System.out.println(String.format("Chain 1 Hash: %s", Objects.requireNonNull(chain1.getHead()).getHash()));
		System.out.println(String.format("Chain 2 Hash: %s", Objects.requireNonNull(chain2.getHead()).getHash()));
		System.out.println(
				String.format("Chains Are In Sync: %s", Objects.equals(chain1.getHead().getHash(), chain2.getHead().getHash())));
		...
		for (Block block : chain1.getChain()) {
		...
		List merkleTree = headBlock.merkleTree();
		assertEquals(headBlock.getMerkleRoot(), merkleTree.get(merkleTree.size() - 1));
